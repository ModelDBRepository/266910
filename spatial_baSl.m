function [Lt,post,ta,blk_id]=spatial_baSl(N,delta,din,dout,Ek,noise_factor,L,pup,pdw)
...
p_col=[0.96,0.55,0.34;0.56,0.69,0.85];
...
%del=round(find(V<(mm-Ek),1,'first')*0)
del=1
...
for i=1:ns*(sd^2)
    zd=floor((i-1)/(sd^2))+1;
    pop(i)=max(round(N/sd^2*NClayers(zd)),1);
end
tc=cumsum(pop);
root=[0 tc(1:end-1)];
...
for ly=1:ns
    ...
    alpha_true([1:ni(end)-del+1])=hi1([del:ni(end)])+0.01*rand(1,ni(end));
    alpha_true=alpha_true/sum(alpha_true);
    sa=sum(alpha_true.*[0:ni(end)-1]);
    la=length(alpha_true);
    
    iota=binopdf([0:la-1],m-1,rho);
    la=length(alpha_true); nsf=10;
    alpha_noise=kron(0.5*(10^-1)*rand(ceil(la/nsf),1),ones(nsf,1));
    gamma{ly}=PN/(PN-m)*(alpha_true+alpha_noise(1:la))-m/(PN-m)*iota;
...
for i=1:ns*(sd^2)
    ...
    for j=1:ns*(sd^2)
        if i==j
            ...
            tic;
            %[Lr,posr]=sba_dir2L(offp,m,gamma{zd},rho,delta,[1/sd 1/sd NClayers(zd)].*dims+overlap,noise_factor);
            [Lr,posr]=sba_dir2LS(offp,m,gamma{zd},rho,delta,[1 1 NClayers(zd)].*dims,noise_factor);
            toc
            if size(Lr,2)==2
                %tp=[Offx(xd,yd)+posr(:,1)-overlap(1)/2,Offy(xd,yd)+posr(:,2)-overlap(2)/2,Offz(zd)+posr(:,3)-overlap(3)/2];
                tp=[posr(:,1),posr(:,2),Offz(zd)+posr(:,3)];
                post=[post; tp];
